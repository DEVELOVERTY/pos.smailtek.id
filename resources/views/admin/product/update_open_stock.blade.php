...
<div class="page-content">
    <div class="container-fluid">
        ...
        <div class="row match-height">
            <form id="" method="POST" action="{{ route('store.opening','update') }}" enctype="multipart/form-data" class="col-md-12 col-12">
                @csrf
                                    ...
                                    <div class="row">
                                        <div class="col-6">
                                            ...
                                            <div class="input-group mb-3">
                                                ...
                                                <input type="hidden" name="id" value="{{$getOpening->id}}">
                                                <input type="text" class="form-control" value="{{ old('ref_no',$getOpening->ref_no) }}" id="product_sku" name="ref_no" readonly >
                                        ...
                                        <div class="col-12">
                                            <div class="table-responsive po_items">
                                                <div class="table-responsive">
                                                    <table class="table table-bordered" id="table-1">
                                                        ...
                                                        <tbody id="defaulth">
                                                            ...
                                                            $total = 0; 
                                                            @endphp
                                                            @foreach ($getOpening->purchase as $v)
                                                            @php
                                                            $subtotal = 0;
                                                            ...
                                                            @endphp
                                                            <tr>
                                                                <td>{{ $v->variation->product->name ?? ''}} - {{ $v->variation->name }}</td>
                                                                <td>
                                                                    <input type="hidden" name="variation_id[]" id="variationID" value="{{$v->variation->id}}">
                                                                    <input type="number" class="form-control qty_opening" name="qty_opening[]" id="{{ $v->variation->id }}" min="1" value="{{number_format($v->quantity)}}">
                                                                </td>
                                                                <td>{{number_format($v->variation->purchase_price)}}</td>
                                                                <td width="15%">
                                                                    <input type="hidden" id="pricing-{{$v->variation->id}}" name="pricing[]" value="{{$v->variation->purchase_price}}">
                                                                    <input type="text" name="opening_subtotal[]" class="form-control opening_subtotal" id="subtotal{{ $v->variation->id }}" value="{{number_format($subtotal)}}" readonly>
                                                                </td>
                                                            </tr>
                                                            @endforeach
                                                ...
                                                <br><br>
                                                <h5 class="float-end" style="margin-top: -20px">{{__('purchase.net_total')}}: <span id="amount_total"> {{number_format($total)}} </span> </h5>
                                                <input type="hidden" name="amount_total" id="total_amount" value="{{$total}}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
